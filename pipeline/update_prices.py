...
from build_maps import build_maps_payload  # noqa: E402
from linkage import METHOD_NOTE, build_linkage  # noqa: E402
from theme_config import SECTOR_IDS, data_dir, sector_config, slug_to_module  # noqa: E402
...
def extra_symbols(theme: str) -> dict[str, tuple[str, str]]:
    cfg = sector_config(theme)
    sym = cfg["benchmark"]["symbol"]
    name = cfg["benchmark"]["name"]
    return {sym: (name, "us")}


def probe_symbol(theme: str) -> tuple[str, str]:
    cfg = sector_config(theme)
    p = cfg["probe"]
    return (p["code"], p["market"])


def probe_fetchers(fetchers: list, probe: tuple[str, str]) -> tuple[list, list[str]]:
...
def build_themes_perf(quotes: dict, last_updated: str, macro: list) -> dict:
    year = last_updated[:4]
    themes = []
    for m in macro:
        codes: dict[str, str] = {}
...
def build_themes_detail(quotes: dict, last_updated: str, macro: list) -> dict:
...
def build_indices(quotes: dict, last_updated: str, extra: dict) -> dict:
    """指数データ(SOX 等)。無料で取得できる範囲の市場指標。"""
    year = last_updated[:4]
    indices = []
    for sym, (name, _market) in extra.items():
        q = quotes.get(sym)
        if not q or not q["daily"]:
            continue
        daily = q["daily"]
...
def build_timeline_stats(themes_detail: dict, last_updated: str, data_dir_path: Path) -> dict:
    """地政学タイムラインの各イベント後の関連テーマ株価反応(+5営業日)。

    timeline.json の tags(テーマ key)と日付(YYYY-MM-DD のもののみ)を使い、
    テーマ累積騰落率系列からイベント直前終値→5営業日後の変化率を計算する。
    """
    timeline_path = data_dir_path / "timeline.json"
...
def run_for_theme(theme: str, args: argparse.Namespace) -> None:
    import importlib

    mod_name = slug_to_module(theme)
    theme_mod = importlib.import_module(f"themes.{mod_name}")
    macro = theme_mod.MACRO
    all_symbols_fn = theme_mod.all_symbols
    extra = extra_symbols(theme)
    probe = probe_symbol(theme)
    ...
    fetchers, sources_disabled = probe_fetchers(fetchers, probe)
    if not fetchers:
        print("!! 全ソースが事前チェックNG。前回データを維持して異常終了します。", file=sys.stderr)
        sys.exit(1)

    symbols = all_symbols_fn()
    ...
    themes_perf = build_themes_perf(quotes, last_updated, macro)
    themes_detail = build_themes_detail(quotes, last_updated, macro)
    indices = build_indices(quotes, last_updated, extra)
    timeline_stats = build_timeline_stats(themes_detail, last_updated, out_dir)
    cfg = sector_config(theme)
    if cfg.get("linkage", {}).get("enabled", True):
    ...
    for fname, payload in outputs.items():
        ...
        print(f"wrote {theme}/{path.name} ({path.stat().st_size / 1024:.0f} KB)")

    print(
        f"完了 [{theme}]: {len(quotes)}/{len(symbols)} 銘柄(失敗 {len(failed)}) / 基準日 {last_updated}"
    )
