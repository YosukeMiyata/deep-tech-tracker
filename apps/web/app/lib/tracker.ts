import { fmtPct } from "~/lib/data-types";
...
export function createTracker(data: SectorDataBundle) {
  const pricesData = data.prices as unknown as {
    last_updated: string;
    quotes: Record<string, Quote>;
  };
  const quotes = pricesData.quotes;
  const macros = (data.themesMaster as { macro: MacroTheme[] }).macro;
  const flowJson = data.flow;
  const linkageJson = data.linkage;
  const processJson = data.process;
  const stockTagsJson = data.stockTags;
  const trackerLastUpdated = pricesData.last_updated;
  ...
  function rankedThemes(period: TrackerPeriodId, market: TrackerMarket): RankedTheme[] {
    return macros
      .map((m, idx) => {
  ...
  function subAvg(sub: SubTheme, period: TrackerPeriodId, market: TrackerMarket): number | null {
  ...
  function searchByTag(query: string, limit = 20): TagSearchHit[] {
  ...
  function searchStocks(query: string, limit = 15): SearchHit[] {
    const q = query.trim().toLowerCase();
    if (!q) {
      return [];
    }
  ...
  function stockChgPct(symbol: string): number | null {
    return chgPct(symbol);
  }

  function stockPeriodRet(symbol: string, period: TrackerPeriodId): number | null {
    return symRet(symbol, period);
  }

  function stockVolRatio(symbol: string): number | null {
    const daily = quotes[symbol]?.daily;
    if (!daily || daily.length < 2) {
      return null;
    }
  ...
  function formatTrackerPct(v: number | null | undefined): string {
    return fmtPct(v);
  }

  function sparkFromSeries(series: [string, number][]): number[] {
    return series.map(([, v]) => v);
  }

  function stockThemes(symbol: string): { macroIdx: number; label: string }[] {
  ...
  function stockTags(symbol: string): string[] {
    return (stockTagsJson as { tags: Record<string, string[]> }).tags[symbol] ?? [];
  }

  function stockLinkageHits(symbol: string): StockLinkageHit[] {
  ...
  function stockRow(symbol: string): StockRow | null {
    const q = quotes[symbol];
    if (!q?.daily?.length) {
      return null;
    }
  ...
  function allStockRows(): StockRow[] {
  ...
  function linkageGroups(): LinkageGroup[] {
  ...
  function sectorSymbolSet(macroKey: string): Set<string> | null {
  ...
  function subSymbolSet(macroKey: string, subName: string): Set<string> | null {
    const m = macros.find((x) => x.key === macroKey);
  ...
  function speProcessSymbolSet(procKey: keyof typeof SPE_PROCESS): Set<string> | null {
  ...
  function macroSectorOptions(): { id: string; label: string }[] {
