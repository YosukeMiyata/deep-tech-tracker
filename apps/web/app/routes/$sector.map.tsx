...
function Chips({ codes, data }: { codes: string[]; data: ReturnType<typeof useSector>["data"] }) {
  return (
    <>
      {codes.map((code) => (
        <span
...
function StepRow({ step, data }: { step: SupplyStep; data: ReturnType<typeof useSector>["data"] }) {
  return (
    <div className="border-line border-b py-3 last:border-b-0 md:py-3.5">
      ...
      {step.equip?.length ? (
        <div className="mb-1.5 md:mb-2">
          ...
          <Chips codes={step.equip} data={data} />
        </div>
      ) : null}
      {step.material?.length ? (
        <div className="mb-1.5 md:mb-2">
          <span className="type-mono-accent mr-2 font-bold">材料</span>
          <Chips codes={step.material} data={data} />
        </div>
      ) : null}
      {step.stocks?.length ? (
        <div className="mb-1.5 md:mb-2">
          <Chips codes={step.stocks} data={data} />
        </div>
      ) : null}
    </div>
  );
...
export default function MapPage() {
  ...
  const supplyStages = stages(data);
  ...
  return (
    <>
      ...
      />
      {supplyStages.map((stage, i) => {
        const count = stageNewsCount(data, stage);
        return (
            ...
            <details open={i === 0} className="mb-2 md:mb-3">
              ...
              <div className="mx-1 mt-[-8px] rounded-b-card border border-line border-t-0 bg-panel2 px-4 pt-4 pb-4 md:px-6 md:pt-5 md:pb-5">
                ...
                {stage.steps.map((step) => (
                  <StepRow key={step.name} step={step} data={data} />
                ))}
