...
import { headlineItems } from "~/lib/headlines";
...
export default function News() {
  ...
  const headlinesMeta = data.headlines as { fetched_at?: string };
  const fetchedLabel = (headlinesMeta.fetched_at ?? "").slice(0, 10);
  const names = themeNames(data);
  const timeline = timelineItems(data);
  const timelineReactions = data.timelineReactions;
  ...
  return (
    <>
      ...
      />
      <HeadlineList items={headlineItems(data)} />
