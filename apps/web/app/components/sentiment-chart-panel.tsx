...
export function SentimentChartPanel() {
  ...
  const themes = data.themesMaster.macro.map((m) => ({ key: m.key, name: m.name, color: m.color }));
  const [themeKey, setThemeKey] = useState<string | null>(null);
  const series = useMemo(() => weeklyThemeSentimentSeries(data, themeKey, 12), [data, themeKey]);
