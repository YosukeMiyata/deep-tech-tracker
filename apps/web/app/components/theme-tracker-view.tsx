...
function SubBlock({
  ...
  const avg = tracker.subAvg(sub, period, market);
    ...
    <div className="border-line border-b py-3 last:border-b-0">
      ...
      {showUs && sub.us.length > 0 ? (
        <>
          ...
          {sub.us.map((row) => {
            const s = tracker.stockRowLabel(row, "us");
            return (
              <StockThemeListRow
                key={s.code}
                symbol={s.code}
                name={s.name}
                market="us"
                pct={tracker.stockPeriodRet(s.code, period)}
      ...
      {showJp && sub.jp.length > 0 ? (
        <>
          ...
          {sub.jp.map((row) => {
            const s = tracker.stockRowLabel(row, "jp");
            return (
              <StockThemeListRow
                key={s.code}
                symbol={s.code}
...
export function ThemeTrackerView() {
  ...
  const rows = useMemo(() => tracker.rankedThemes(period, market), [tracker, period, market]);
          ...
          <>
            ...
            <div className="border-line border-t pt-4 pb-2 md:pt-5">
              ...
              <div className="type-meta mt-2 text-right font-mono md:mt-3">
                データ {tracker.trackerLastUpdated} 時点
              </div>
            </div>
              ...
              {rows.map((t, rank) => (
                ...
                >
                  <summary className="flex cursor-pointer list-none items-center gap-2.5 border-line border-b py-3 [&::-webkit-details-marker]:hidden hover:bg-panel2/35 focus-visible:outline-2 focus-visible:outline-copper focus-visible:outline-offset-2 md:gap-3 md:py-4">
                    ...
                    <Sparkline values={tracker.sparkFromSeries(t.series)} />
                    <div
