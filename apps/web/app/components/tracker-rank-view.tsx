...
import { analyzeStock, BULL_PATTERNS, type StockTechnical } from "~/lib/technical";
import type { StockRow } from "~/lib/tracker";
...
function matchSignals(tech: StockTechnical, signals: Set<string>): boolean {
...
export function RankView({
  ...
  const macros = data.themesMaster.macro;
  ...
  const rows = useMemo(() => {
    let secSet: Set<string> | null = sector === "all" ? null : tracker.sectorSymbolSet(sector);
    if (sector !== "all" && sub !== "all") {
      if (sub.startsWith("proc:")) {
        const pk = sub.slice(5) as "front" | "middle" | "back";
        secSet = tracker.speProcessSymbolSet(pk) ?? secSet;
      } else {
        secSet = tracker.subSymbolSet(sector, sub) ?? secSet;
      }
    }
    ...
    return tracker
      ...
      .flatMap((r) => {
        ...
        const tech = analyzeStock(data, r.symbol);
        if (!tech) {
          return [];
        }
  ...
  }, [tracker, data, sector, sub, market, patterns, signals]);
  ...
  return (
    <>
      ...
      <ChipGroup
        label="セクター(マクロ)で絞る"
        options={tracker.macroSectorOptions()}
        value={sector}
        onChange={(id) => {
          setSector(id);
          setSub("all");
...
function RankRow({
  ...
  row: StockRow & { tech: StockTechnical };
  rank: number;
