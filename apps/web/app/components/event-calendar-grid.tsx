...
function EventListView({
  ...
  const { rangeLabel } = twoWeekCalendarGrid(data);
          ...
          <div key={date}>
            <div
              className={`type-mono-accent md:text-[14px] ${
                isEventAnchorDay(data, date) ? "font-bold text-copper" : "text-ink-2"
              }`}
            >
              {eventDateLabel(date)}({eventWeekdayLabel(date)})
              {isEventAnchorDay(data, date) ? " · 基準日" : ""}
            </div>
...
export function EventCalendarGrid({
  ...
  const { data } = useSector();
  const { weeks, rangeLabel } = twoWeekCalendarGrid(data);
