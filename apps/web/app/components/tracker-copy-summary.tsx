...
function buildSummary(
  data: ReturnType<typeof useSector>["data"],
  sectorName: string,
  tracker: ReturnType<typeof useTracker>,
): string {
  const quotes = (data.prices as { quotes: Record<string, { name: string }> }).quotes;
  const qs = Object.entries(quotes)
    .map(([sym, row]) => {
      const tech = analyzeStock(data, sym);
      if (!tech) {
        return null;
      }
  ...
  let txt = "";
  txt += `あなたは${sectorName}株に詳しい投資分析アシスタントです。以下は${sectorName}テーマトラッカーから出力した本日の市場データです。デイトレ候補・スイング候補・連動セクター・過熱警告・資金の流れを分析してください。\n\n`;
  txt += `【${sectorName}トラッカー 市場サマリー】データ日付: ${tracker.trackerLastUpdated.slice(0, 10)}\n\n`;
  ...
  txt += "\n━━━ 米国株トリガー × 連動 ━━━\n";
  for (const g of tracker.linkageGroups().slice(0, 8)) {
...
export function TrackerCopySummary() {
  ...
  const copy = async () => {
    try {
      await navigator.clipboard.writeText(buildSummary(data, sector.name, tracker));
      setCopied(true);
