    ...
    <>
      {stocks.map((s) => (
        <StockThemeListRow
          ...
          name={s.name}
          market={s.market}
          pct={procVal(data, s.code, period)}
          ...
          <div className="border-line border-t pb-2 pt-1">
            {step.groups.map((g) => (
              <div key={g.label} className="mb-2 last:mb-0">
                ...
                <StockList stocks={g.stocks} period={period} onPick={onPickStock} data={data} />
              </div>
            ))}
          </div>
        ) : (
...
export function ProcView({ onPickStock }: { onPickStock: (symbol: string) => void }) {
  ...
  const steps = (data.process as { steps: ProcStep[] }).steps;
