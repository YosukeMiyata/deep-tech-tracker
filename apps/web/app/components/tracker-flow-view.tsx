...
function FlowChip({
  ...
  chgPct: (code: string) => number | null;
}) {
  const chg = chgPct(item.code);
          ...
          {step.roles.map((role) => (
            <div key={role.label} className="frole">
              ...
              <div className="fchips">
                {role.items.map((item) => (
                  <FlowChip key={item.code} item={item} onPick={onPickStock} chgPct={chgPct} />
                ))}
              </div>
            </div>
          ))}
...
export function FlowView({ onPickStock }: { onPickStock: (symbol: string) => void }) {
  ...
  const stages = (data.flow as { stages: FlowStage[] }).stages;
