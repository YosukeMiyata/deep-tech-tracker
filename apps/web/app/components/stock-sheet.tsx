...
export function StockSheet({
  ...
  const row = tracker.stockRow(symbol);
  if (!row) {
    return null;
  }

  const themes = tracker.stockThemes(symbol);
  const tags = tracker.stockTags(symbol);
  const linkHits = tracker.stockLinkageHits(symbol);
  const tech = analyzeStock(data, symbol);
      ...
      >
        ...
        <div className="mb-5 grid grid-cols-3 gap-2 md:mb-6 md:gap-2.5">
          {TRACKER_PERIODS.map((p) => {
            const v = tracker.stockPeriodRet(symbol, p.id as TrackerPeriodId);
            return (
              <div
                key={p.id}
