...
export function ThemeSummary() {
  ...
  return (
    <>
      <SectionTitle
        title="テーマ騰落サマリー"
        note={`12マクロテーマの前日比・年初来・${benchmarkName}比(α)。構成銘柄の単純平均`}
        info={{
          ...
          content: (
            <>
              ...
              <ul className="list-disc space-y-1 pl-4">
                ...
                <li>
                  <strong className="font-medium text-ink">α（{benchmarkName}比）</strong>
                  ：テーマの年初来騰落率から{benchmarkName}
                  の年初来騰落率を引いた値。指数に対する相対パフォーマンス
                </li>
                <li>
          ...
          return (
            ...
            >
              ...
              <div className="mt-1.5 flex w-full flex-wrap items-center justify-end gap-x-2 gap-y-1 md:mt-0 md:ml-auto md:w-auto md:shrink-0 md:flex-nowrap md:justify-start md:gap-x-3">
                ...
                <span
                  className={`type-mono-value md:w-[4rem] md:text-right ${pctColor(t.benchmarkAlpha)}`}
                  title={`年初来騰落率 − ${benchmarkName}年初来`}
                >
                  {t.benchmarkAlpha === null ? "—" : fmtPct(t.benchmarkAlpha, 0)}
